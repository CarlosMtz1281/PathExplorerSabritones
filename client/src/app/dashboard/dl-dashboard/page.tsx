...
const DashboardDL = () => {
    ...
    const toggleTooltip = (postulationId: number) => {
        setVisibleTooltipId(visibleTooltipId === postulationId ? null : postulationId);
    };
    ...
    const userHasCertificate = (user: PostulationUser, certificateId: number): boolean => {
        return user.userCertificates?.some(c => c.certificateId === certificateId) || false;
    };
    ...
    const handleScheduleMeeting = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE}/dl/scheduleMeeting`, {
    ...
    const handleRejectPostulation = async (postulationId: number) => {
        try {
            ...
            const confirmReject = window.confirm("¿Estás seguro que deseas rechazar esta postulación?");
    ...
    const handleAcceptPostulation = async (postulationId: number) => {
        try {
            ...
            const confirmAccept = window.confirm("¿Estás seguro que deseas aceptar esta postulación? Esto invalidará todas las demás postulaciones para este puesto.");
    ...
    return (
                                                        ...
                                                        <div className="text-right">
                                                            <div className="flex items-center justify-end mb-4">
                                                                ...
                                                                <div className="relative" ref={tooltipRef}>
                                                                    <button
                                                                        onClick={() => toggleTooltip(postulation.postulationId)}
                                                                        className="btn btn-sm btn-circle btn-ghost"
                                                                    >
                                                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                                    ...
                                                                    {visibleTooltipId === postulation.postulationId && (
                                                                        <div className="absolute right-0 z-10 w-96 p-4 mt-2 bg-base-100 rounded-lg shadow-xl border-2 border-base-300">
                                                                        <div className="text-sm text-left">                                                                            
                                                                            {/* Skills requeridos */}
                                                                            <div className="mb-3">
                                                                            <h5 className="font-semibold mb-1">Habilidades requeridas:</h5>
                                                                            {postulation.position.requiredSkills && postulation.position.requiredSkills.length > 0 ? (
                                                                                <div className="flex flex-wrap gap-1">
                                                                                {postulation.position.requiredSkills.map(skill => (
                                                                                    <span 
                                                                                    key={skill.skillId} 
                                                                                    className={`badge badge-sm ${
                                                                                        userHasSkill(postulation.user, skill.skillId) 
                                                                                        ? 'badge-success' 
                                                                                        : 'badge-failure'
                                                                                    }`}
                                                                                    >
                                                                                ...
                                                                                <p className="text-gray-500">No se requieren habilidades específicas</p>
                                                                            )}
                                                                            </div>
                                                                            
                                                                            {/* Certificados requeridos */}
                                                                            <div>
                                                                            <h5 className="font-semibold mb-1">Certificados requeridos:</h5>
                                                                            {postulation.position.requiredCertificates && postulation.position.requiredCertificates.length > 0 ? (
                                                                                <div className="flex flex-wrap gap-1">
                                                                                {postulation.position.requiredCertificates.map(cert => (
                                                                                    <span 
                                                                                    key={cert.certificateId} 
                                                                                    className={`badge badge-sm ${
                                                                                        userHasCertificate(postulation.user, cert.certificateId) 
                                                                                        ? 'badge-success' 
                                                                                        : 'badge-failure'
                                                                                    }`}
                                                                                    >
                                                                                ...
                                                                                <p className="text-gray-500">No se requieren certificados específicos</p>
                                                                            )}
                                                                            </div>
                                                                        </div>
                                                                        </div>
                                                            ...
                                                            <div className="flex gap-1">
                                                                {postulation.meetings.length > 0 ? (
                                                                    <div className="dropdown dropdown-end">
                                                                        ...
                                                                        >
                                                                        <h4 className="font-bold mb-2">Reunión programada</h4>
                                                                        <p className="mb-2">
                                                                            <span className="font-semibold">Fecha:</span> {new Date(postulation.meetings[0].meetingDate!).toLocaleString()}
                                                                        </p>
                                                                        <a 
                                                                            href={postulation.meetings[0].meetingLink} 
                                                                            target="_blank"
                                                                ...
                                                                <button 
                                                                    ...
                                                                    onClick={() => handleRejectPostulation(postulation.postulationId)}
                                                                    >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                ...
                                                                <button 
                                                                    ...
                                                                    onClick={() => handleAcceptPostulation(postulation.postulationId)}
                                                                >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {meetingModalOpen && selectedPostulation && (
            <div className="modal modal-open">
                ...
                <h3 className="font-bold text-2xl mb-6 text-center">Agendar reunión con {selectedPostulation.user.name}</h3>
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="w-full lg:w-1/2">
                        <div className="card bg-base-200 p-4">
                            ...
                            {['D', 'L', 'M', 'M', 'J', 'V', 'S'].map((day, i) => (
                                <div key={`day-${i}`} className="font-bold text-sm p-2">{day}</div>
                            ))}
                    ...
                    <div className="card bg-base-200 p-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            const fullTimeString = `${hour}:${minute.toString().padStart(2, '0')}:00`;
                            
                            return (
                                <button
                                key={`time-${i}`}
                                className={`btn btn-sm ${
                                    selectedTime === fullTimeString ? 'btn-accent' : 'btn-outline'
                                }`}
                ...
                <div className="mt-6 bg-base-100 p-4 rounded-box border border-base-300">
                    <h4 className="font-semibold text-lg mb-3">Resumen de la reunión</h4>
                    <div className="space-y-2">
                    <div className="flex items-center">
                        <span className="w-24 font-medium">Fecha:</span>
                        <span className={selectedDate ? "text-primary" : "text-gray-500"}>
                        {selectedDate ? formatSelectedDate(selectedDate) : 'No seleccionada'}
                        </span>
                    </div>
                    <div className="flex items-center">
                        <span className="w-24 font-medium">Hora:</span>
                        <span className={selectedTime ? "text-primary" : "text-gray-500"}>
                        {selectedTime ? 
                            `${selectedTime.split(':')[0]}:${selectedTime.split(':')[1]} ${parseInt(selectedTime.split(':')[0]) >= 12 ? 'PM' : 'AM'}` : 
                            'No seleccionada'}
                        </span>
                    </div>
                    <div className="flex items-center">
                        <span className="w-24 font-medium">Enlace:</span>
                        <span className="text-primary">Se generará automáticamente</span>
                    </div>
                    </div>
                </div>
