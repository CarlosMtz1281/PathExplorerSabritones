...
class CoursesRecommender:
    ...
    def train(self, courses: List[Dict]):
        """Build skill matrix for all courses"""
        self.skills_ids = list(
            {skill["skill_id"] for course in courses for skill in course["skills"]}
        )

        self.courses_ids = [course["id"] for course in courses]

        self.skill_matrix = np.zeros((len(courses), len(self.skills_ids)))
        ...
        self.course_sim_matrix = cosine_similarity(self.skill_matrix)
    ...
    def recommend(
        ...
        """Get top course recommendations using MMR diversification"""
        ...
        candidate_indices = [
            idx
            for idx, course_id in enumerate(self.courses_ids)
            if course_id not in exclude_course_ids
        ]
        ...
        while len(recommendations) < top_n and candidate_indices:
            ...
            for idx in candidate_indices:
                ...
                if not selected_indices:
                    ...
                    max_sim = np.max(self.course_sim_matrix[idx, selected_indices])
                    mmr = (diversity_lambda * current_sim) - (
            ...
            course_id = self.courses_ids[best_idx]
